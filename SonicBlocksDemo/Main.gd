...
func _on_LocalButton_pressed() -> void:
	main_menu.visible = false
	players[1].input_prefix = "player2_"
	SyncManager.network_adaptor = DummyNetworkAdaptor.new()
	SyncManager.start()

func _on_ServerButton_pressed() -> void:
	players[0].camera.current = true
	players[1].camera.current = false
...
func _on_ClientButton_pressed() -> void:
	players[0].camera.current = false
	players[1].camera.current = true
	_start_client()
...
func _on_server_connected() -> void:
	for x in players.size():
		if x == 0: continue
		players[x].set_multiplayer_authority(multiplayer.get_unique_id())
...
func register_player(options: Dictionary = {}) -> void:
	...
	for x in players.size():
		if x == 0: continue
		players[x].set_multiplayer_authority(peer_id)
...
func setup_match_for_replay(my_peer_id: int, peer_ids: Array, _match_info: Dictionary) -> void:
	...
	for x in players.size():
		if x == 0: continue
		players[x].set_multiplayer_authority(client_peer_id)
